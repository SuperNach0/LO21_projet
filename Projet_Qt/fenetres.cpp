...
fenetre_creation_relation::fenetre_creation_relation(QWidget *parent)//:couples(0)
{
    this->setWindowModality(Qt::ApplicationModal); //pour que la fenetre parente ne soit pas utilisable quand celle ci est ouverte
    m_parent = parent;
...
void fenetre_creation_relation::save_relation()
{
    ...
    std::cout<< "OKKKK\n";
    //les couples sont stockés dans le vector "couples", on les ajoute tous à la relation "nouvelle_relation"
    for (unsigned int i =0;i<couples.size();i++)
    {
        //std::cout<< m_liste_couples->currentItem()->text().toStdString() << std::endl;
        nouvelle_relation->addCouple(couples[i]);
    }
    std::cout<< "OKKKK222222\n";
...
void fenetre_creation_relation::save_couple()
{
    ...
    Couple* nouveau = new Couple(m.getNote(m_notes_gauche->currentItem()->text().toStdString()),m.getNote(m_notes_droite->currentItem()->text().toStdString()),label);
    couples.push_back(*nouveau);

    for (int i=0; i<couples.size();i++)
    {
        std::cout << "\nlabel = " << couples[i].getLabel();
    }
}
