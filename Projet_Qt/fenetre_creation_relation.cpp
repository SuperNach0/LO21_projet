...
fenetre_creation_relation::fenetre_creation_relation(QWidget *parent):m_couples(0)
{
    ...
    for (unsigned int i = 0; i<m1.getobjets().size();i++)
    {

        m_notes_gauche->addItem(QString::fromStdString(m1.getobjets()[i]->getID()));
        m_notes_droite->addItem(QString::fromStdString(m1.getobjets()[i]->getID()));
    }
